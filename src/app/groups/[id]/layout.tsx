...
async function getGroupHeader(id: string) {
  // TODO: /api/groups/:id GET bağla
  return {
    _id: id,
    name: id === "demo-1" ? "StuZync Devs" : "AI Study Club",
        ...
        <div className="p-5">
          <div className="flex items-end gap-4">
            ...
            {g.isAdmin && (
              <div className="flex gap-2">
                <a href={`/groups/${g._id}/settings`} className="btn btn-ghost">Settings</a>
                <a href={`/groups/${g._id}/manage`} className="btn btn-ghost">Manage</a>
              </div>
            )}
          </div>
          ...
          <nav className="tabs mt-5">
            {[
              ...
              <a key={t.href} href={t.href} className="tab">{t.label}</a>
            ))}
          </nav>
        </div>
      </div>
